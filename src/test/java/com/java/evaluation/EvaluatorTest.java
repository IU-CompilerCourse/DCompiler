...
public class EvaluatorTest {
    ...
    @BeforeAll
    static void tearUp() {
        Path testInputDirectory = Path.of("src", "test", "resources", "evaluator-test-cases");

        Path testOutputDirectory =
            Path.of("src", "test", "resources", "evaluator-test-output");
    ...
    void givenTestCases_whenLexerUsed_thenCorrectOutput() {
        ...
        TESTCASES_INPUT.forEach((key, value) -> {
            ...
            try {
                ...
                analyzerOutput = new Analyzer().analyze(ast);
            } catch (Exception e) {
                isOK = false;
            ...
            if (isOK) {
                ...
                System.out.println("\uD83C\uDF32 Checked and optimized by: Analyzer\n");
                ...
                try {
                    ...
                    try {
                        analyzerOutput.ast().accept(new Evaluator());
                    } catch (Exception any) {
                        toWrite = any.getMessage();
                    }
                    ...
                    TESTCASES_ACTUAL_OUTPUT.put(
                        key,
                        Files.readString(tempOutputFile) + (toWrite != null ? toWrite : "")
                    );
