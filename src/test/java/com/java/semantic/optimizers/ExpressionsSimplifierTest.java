...
public class ExpressionsSimplifierTest {
    ...
    void givenTestCases_whenLexerUsed_thenCorrectOutput() {
        ...
        TESTCASES_INPUT.forEach((key, value) -> {
            ...
            try {
                ...
                var simplifier = new ExpressionsSimplifier();
                ast.accept(simplifier);
                analyzerOutput = new AnalyzerOutput(ast, Map.of(
                    simplifier.getClass().getSimpleName(),
                    simplifier.getErrors()
                ));
            } catch (Exception e) {
                isOK = false;
            ...
            if (isOK) {
                ...
                System.out.println("\uD83C\uDF32 Optimized by: ExpressionsSimplifier\n");
